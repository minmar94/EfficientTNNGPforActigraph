...
Rcpp::List tNngpCollapsed_NAda(const Eigen::Map<ArrayXd> t, const MapVecd Z, 
                               ...
                               ColVecd beta0, ColVecd logtheta0, 
  ...
  // Data infos
  const int& n = Z.size();
  const int& ncov = X.cols();
  const Matd& tX = X.adjoint();
  const Matd& tXX = (tX*X);
  const ColVecd& tXZ = (tX*Z);
  ...
  // Gibbs
  const Matd& Fmat = vB.inverse();
  const ColVecd& fmat = vB.ldlt().solve(muB);
  ...
  for (int k=0; k<nInd; k++){
    ...
    // v for Gibbs on beta
    v.block(cselv, 0,  nk, ncov) = solverO.solve(X.block(cselv, 0, nk, ncov));
    cselv += nk;
  }
  ...
  for(int m = 1; m < M; m++){
    ...
    for (int k=0; k<nInd; k++){
      ...
      #pragma omp critical
      {
        ...
        vNew.block(cselsk, 0,  nk, ncov) = solverO1.solve(X.block(cselsk, 0, nk, ncov));
      }
    }
    ...
    // Computing b and B
    const ColVecd& b = (tXZ)/tau20 - (vtZ)/(tau20*tau20) + fmat;
    const Matd& B = tXX/tau20 - (tXv)/(tau20*tau20) + Fmat;
    
    // Simulation
    LLT<Matd> cholB(B);
    Matd LB = cholB.matrixL();
    beta0 = cholB.solve(b) + 
      LB.triangularView<Lower>().solve(zrnorm(ncov, 1));
    betas.col(m) = beta0;
  ...
  string cname = fileName + std::to_string(M/verbose) + "_chains.txt";
  chainsFile.open(cname.c_str());
  MatrixXd curChains(lthetas.rows()+betas.rows(), (M-1)%verbose+1);
  curChains << lthetas.rightCols((M-1)%verbose+1), betas.rightCols((M-1)%verbose+1);
  chainsFile << curChains.format(ChainFmt) << "\n";
  chainsFile.close();
  
  Rcpp::Rcout << "Written chains" << std::endl;
  // Last predictions
  const ColVecd& mm = X*beta0;
  const ColVecd& r = Z - mm;
  ColVecd v0(n);
  ...
  // Loop over individuals
  #pragma omp parallel for private(solverO, indsk, nk, cselsk, tempV0) schedule(auto)
  for (int k=0; k<nInd; k++){
    ...
    // v0 piece computation and predictions
    tempV0 = solverO.solve(r.segment(cselsk, nk));
    const SpMat L = solverO.matrixL();
  ...
  return Rcpp::List::create(thetas, betas, acc, wPreds, loglik);
}

// [[Rcpp::export]]
Rcpp::List tNngpCollapsed_Preds(const Eigen::Map<ArrayXd> t, 
                                const Eigen::Map<ArrayXd> tPred, const MapMatd Xpred,
                                const int& neigh, const int& index,
                                const Eigen::Map<ArrayXXd> betas, const Eigen::Map<ArrayXXd> thetas, 
  ...
  for(int m = 0; m < M; m++){
    ...
    // Loop on value to predict (after first observed)
    // #pragma omp parallel for private(dists0i, covs0i, CN0i) schedule(auto)
    ...
    for(int i = 0; i<nPred; i++){
      ...
      #pragma omp critical
      {
        ...
        // Process
        yPredsOut(i, m) = Xpred.row(i)*(betam) + wPredsOut(i, m) + sqrt(tau2)*zigg.norm();
      }
    }
  }
