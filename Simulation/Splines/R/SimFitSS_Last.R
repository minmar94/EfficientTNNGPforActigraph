...
Rcpp::sourceCpp("Simulation/Splines/Rcpp/tNngpColl_Shrink.cpp")
...
today <- Sys.time() %>% str_replace_all(., " ", "_") %>% 
  str_remove_all(., "-|:")
filenm <- paste("Simulation/Splines/WS/fitSSplineGRW", M, length(uinds), n, ncov, nspl, pte, today, sep="_")
...
# Parameters
chains <- rbind(betas[, (burnIn):M], etas[, (burnIn):M], thetas[, (burnIn):M], 
                lambdas[(burnIn):M])
...
betasp <- betas[, (M-verb+1):(M)]
etasp <- etas[, (M-verb+1):(M)]
thetasp <- thetas[, (M-verb+1):(M)]

yPredsInLast <- eigenYPred(as.matrix(XStr), rbind(betasp, etasp), wPredsInLast, thetasp[3, ])
