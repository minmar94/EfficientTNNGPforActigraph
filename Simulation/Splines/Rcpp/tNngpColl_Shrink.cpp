...
//  - alphaS, betaS, alphaPhi, betaPhi, alphaT, betaT, muB, VB, muE, vE, alphal0, betal0 = priors' hyperparameters
...
Rcpp::List tNngpCollapsed_NAda(const Eigen::Map<ArrayXd> t, const MapVecd Z, 
                               ...
                               const MapVecd muB, const MapMatd vB, const MapVecd muE, const MapMatd vE,
  ...
  Matd vEinv = vE.inverse();
  ColVecd vEinvmuE = vE.ldlt().solve(muE);
  
  Matd FmatE = lambda20*vEinv;
  ColVecd fmatE = lambda20*vEinvmuE;
  ...
  for(int m = 1; m < M; m++){
    if (m%verbose==0)
    {
      ...
      chainsFile.open(cname.c_str(), ios::out | ios::app | ios::binary);
    ...
    const double& invbetal = 1/(betal0 + eta0.dot(eta0)/2);
    ...
    FmatE = lambda20*vEinv;
    fmatE = lambda20*vEinvmuE;
  ...
  string pname = fileName+std::to_string(M/verbose)+"_preds.txt";
  predsFile.open(pname.c_str(), ios::out | ios::app | ios::binary);
  predsFile << wPreds.format(ChainFmt) << "\n";
  predsFile.close();
